...
class MMRotatE(Model):
    ...
    def __init__(self, ent_tot, rel_tot, dim=100, img_emb=None, img_dim=4096,
                 norm_flag=True, margin=None, epsilon=None, test_mode='lp', beta=None):
        super(MMRotatE, self).__init__(ent_tot, rel_tot)

        self.dim = dim // 2  # Use half dimension for complex number representation
        self.margin = margin
    ...
    def _calc(self, h, t, r, mode):
        ...
        if mode != 'normal':
            re_score = re_score.view(-1, r.shape[0], re_score.shape[-1])
            im_score = im_score.view(-1, r.shape[0], im_score.shape[-1])
            re_tail = re_tail.view(-1, r.shape[0], re_tail.shape[-1])
            im_tail = im_tail.view(-1, r.shape[0], im_tail.shape[-1])
